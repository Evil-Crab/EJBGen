...
WaitingBean Waiting;
    ...
    private void triggerTransition(final String key)
    {
        ...
        } else if(checkExtra(key))
        {
            ...
            String token;
token = Returning.getFirstToken(key);
Returning.removeToken(key, token);
Waiting.createToken(key);
    ...
    private boolean blockInputs(String key) {
        ...
        blocked = blocked && Returning.blockPosition(key, name);
    ...
    private void unblockInputs(String key) {
        Returning.unblockPosition(key, name);
