...
SearchingBean Searching;
    ...
    private void triggerTransition(final String key)
    {
        ...
        } else if(checkExtra(key))
        {
            ...
            String token;
token = Starting.getFirstToken(key);
Starting.removeToken(key, token);
Searching.createToken(key);
    ...
    private boolean blockInputs(String key) {
        ...
        blocked = blocked && Starting.blockPosition(key, name);
    ...
    private void unblockInputs(String key) {
        Starting.unblockPosition(key, name);
