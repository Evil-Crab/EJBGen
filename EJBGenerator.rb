#Author: Artem Nikolenko, 2013
#Program description: Script for genereating EJB beans from XML model.
...
"
  ...
  (0..systemTransitions.count-1).each do |i|
    ...
    (0..arcsTPIds.count-1).each do |i|
      ...
      constantValue = systemNet.xpath("netSystem/arcsTP[@id='#{arcsTPIds[i]}']/inscription/monomConstants/@value").to_s.delete('#')
      constantMarking = doc.xpath("//tokenNets[@id='#{constantValue}']/@value").to_s.delete('#')
      outputs.store(outputName, outputVariable)
      if (constantMarking!='')
        constants.store(outputName, constantMarking)
      end
    end
    ...
    if (synch.count == 0)
      ...
      (0..constants.count-1).each do |i|
        netManager = doc.xpath("//elementNetMarkeds[@id='#{constants.values[i]}']/ancestor::typeElementNet/@name").to_s
        if (!variablesBlock.include?("#{netManager}ManagerBean"))
          variablesBlock << "@EJB\n#{netManager}ManagerBean #{netManager}Manager;\n"
        end
        triggerBlock << "token = #{netManager}Manager.createToken(\"#{constants.values[i]}\");\n"
        triggerBlock << "#{constants.keys[i]}.addToken(token);\n"
      end
      ...
      (0..nonblackInput.count-1).each do |i|
        ...
        nonblackOutput = outputs.select { |k, v| v == nonblackInput.values[i] }
        if (nonblackOutput.count == 0)
          netManagerType = systemNet.xpath("netSystem/places[@name='#{nonblackInput.keys[0]}']/@type").to_s.delete('#')
          netManager = doc.xpath("//typeElementNet[@id='#{netManagerType}']/@name")
    ...
    extraBlock = ''
    actionBlock = ''
    if (File.exists?(fileDir+transName+'.iea'))
      part = '%import%'
      File.open(fileDir+transName+'.iea', "r") do |infile|
